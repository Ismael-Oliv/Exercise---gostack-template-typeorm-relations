...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductWithNoQuantityAvalible = products.filter(
      product =>
        existentProducts.filter(ExistenProd => ExistenProd.id === product.id)[0]
          .quantity < product.quantity,
    );

    if (findProductWithNoQuantityAvalible.length) {
      throw new AppError(
        `The quantity ${findProductWithNoQuantityAvalible[0].quantity}
          is not available for ${findProductWithNoQuantityAvalible[0].id}`,
      );
    }
